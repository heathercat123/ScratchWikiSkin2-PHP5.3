...
#navigation, .dropdown {
	background-color: <?=htmlspecialchars(str_replace(array(';', '}'), '', $colorPref))?>;
}
</style>
<?php
$wordmark = $this->get('stylepath') . '/' . $this->getSkin()->stylename . '/resources/Scratch-logo-sm.png';
$wordmarkW = 76;
$wordmarkH = 28;
?>
...
foreach ($this->data['personal_urls'] as $key => $tab) {
	...
	if (!array_key_exists('href', $tab)) {
		if (!array_key_exists('class', $tab)) $tab['class'] = array();
	...
	if (strpos($key, 'notifications') !== false) {
		...
		if ($key === 'notifications-alert') $echoUnread = isset($tab['data']['counter-num']) ? $tab['data']['counter-num'] : $echoUnread;
		continue; // don't include the item in personal URLs
	}
	array_push($personalUrls, $this->makeListItem($key, $tab));
}
// ONLY output this if Echo extension is turned on and user is logged in. Otherwise don't put it there.
if (ExtensionRegistry::getInstance()->isLoaded("Echo") && $wgUser->isLoggedIn()) { ?>
...
<?php } ?>
			...
			<li class="link right account-nav">
				<a class="dropdown-toggle" href="<?php if (!$wgUser->isLoggedIn()) { ?><?=SpecialPage::getTitleFor( 'UserLogin' )->getLinkURL() ?><?php } else { ?><?=$user->getUserPage()->getLinkURL()?><?php } ?>">
					<span class="profile-name"><?php if (!$wgUser->isLoggedIn()) { ?><?=wfMessage( 'scratchwikiskin-notloggedin' )->inLanguage( $wgLang )->escaped()?><?php } else { ?><?=htmlspecialchars($user->getName())?><?php } ?></span>
				</a>
				<ul class="dropdown">
...
<?php }
if (!$wgUser->isLoggedIn() && $wgSWS2JoinBox) { ?>
